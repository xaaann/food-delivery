...
.btn-white{background:white;color:#667eea;font-weight:600;}
...
.stat-value{font-size:36px;font-weight:bold;color:#667eea;margin-bottom:5px;}
...
.stock-low{color:#ff4757;font-weight:bold;}
.stock-ok{color:#2ecc71;}
.action-buttons{display:flex;gap:10px;}
.btn-small{padding:5px 10px;font-size:13px;border-radius:5px;text-align:center;}
.btn-edit{background:#667eea;color:white;}
.btn-delete{background:#ff4757;color:white;}
.food-img{width:60px;height:60px;object-fit:cover;border-radius:5px;margin-right:10px;}
...
<strong>⚠️ Low Stock Alert:</strong> <?= count($low_stock) ?> item(s) have less than 10 units in stock. Please restock soon.
</div>
<?php endif; ?>
...
<img src="<?= htmlspecialchars($item['image'] ?: '../assets/default-food.jpg') ?>" class="food-img" alt="Food">
<div>
...
<td><span class="<?= $item['stock'] < 10 ? 'stock-low':'stock-ok' ?>"><?= $item['stock'] ?> units</span></td>
<td>
<span class="status-badge <?= $item['available'] ? 'status-available':'status-unavailable' ?>"><?= $item['available'] ? 'Available':'Unavailable' ?></span>
</td>
<td>
<div class="action-buttons">
<a href="edit_food.php?id=<?= $item['id'] ?>" class="btn-small btn-edit">Edit</a>
<a href="delete_food.php?id=<?= $item['id'] ?>" class="btn-small btn-delete" onclick="return confirm('Are you sure you want to delete this item?')">Delete</a>
</div>
</td>
</tr>
<?php endforeach; ?>
...
<?php else: ?>
<p style="text-align:center;color:#888;padding:40px 0;">No food items yet. Click "Add New Item" to get started!</p>
<?php endif; ?>
</div>
</div>
</body>
